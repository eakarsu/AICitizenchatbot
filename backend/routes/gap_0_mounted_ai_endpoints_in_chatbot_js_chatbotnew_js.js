  ...
  try {
    ...
    if (!process.env.OPENROUTER_API_KEY) {
      return res.status(503).json({
        ...
        feature: 'gap_0_mounted_ai_endpoints_in_chatbot_js_chatbotnew_js',
      });
    }
    const payload = req.body || {};
    const userPrompt = `Context (JSON):\n${JSON.stringify(payload).slice(0, 4000)}\n\nTask: ${ '0 mounted AI endpoints in chatbot.js/chatbotNew.js despite the project name' }\n\nReturn JSON: {"summary": string, "actions": [string], "risks": [string], "confidence": number_0_to_1}.`;
    const system = `You are an expert assistant for the feature: '0 mounted AI endpoints in chatbot.js/chatbotNew.js despite the project name'. Respond with strict JSON.`;
    ...
    const out = { feature: 'gap_0_mounted_ai_endpoints_in_chatbot_js_chatbotnew_js', model: aiResp?.model, parsed, raw: content };
    if (dbPool) {
      try {
        await dbPool.query(
          'INSERT INTO gap_features (feature_key, user_id, input_payload, output_payload) VALUES ($1, $2, $3, $4)',
          ['gap_0_mounted_ai_endpoints_in_chatbot_js_chatbotnew_js', req.user?.id || null, JSON.stringify(payload), JSON.stringify(out)]
        );
...
router.get('/health', (req, res) => res.json({ feature: 'gap_0_mounted_ai_endpoints_in_chatbot_js_chatbotnew_js', ok: true, ai_configured: !!process.env.OPENROUTER_API_KEY }));
...
router.get('/history', async (req, res) => {
  ...
  try {
    ...
    const r = await dbPool.query(
      ...
      ['gap_0_mounted_ai_endpoints_in_chatbot_js_chatbotnew_js']
    );
