...
export default function RealTimeOutageEmergencyBroadcastWithCitywidePage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-real-time-outage-emergency-broadcast-with-citywide/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Real-time outage / emergency broadcast with citywide push notifications</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-real-time-outage-emergency-broadcast-with-citywide/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
