...
export default function FrontendPagesFolderIsEmptyNoSpaUiShippedPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gap-frontend-pages-folder-is-empty-no-spa-ui-shipped/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Frontend pages folder is empty — no SPA UI shipped</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-frontend-pages-folder-is-empty-no-spa-ui-shipped/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
