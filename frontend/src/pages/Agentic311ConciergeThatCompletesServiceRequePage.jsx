...
export default function Agentic311ConciergeThatCompletesServiceRequePage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-agentic-311-concierge-that-completes-service-reque/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Agentic 311 concierge that completes service requests end-to-end with status updates</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-agentic-311-concierge-that-completes-service-reque/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
