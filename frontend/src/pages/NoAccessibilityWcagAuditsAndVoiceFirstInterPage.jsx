...
export default function NoAccessibilityWcagAuditsAndVoiceFirstInterPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gap-no-accessibility-wcag-audits-and-voice-first-inter/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No accessibility (WCAG) audits and voice-first interface</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-accessibility-wcag-audits-and-voice-first-inter/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
